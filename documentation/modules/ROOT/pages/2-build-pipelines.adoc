...
Next, open the `values.yaml` file.

image::gitops-values.png[gitops-values]
...
Switch to the `Source Control` menu in VSCode. Click on `+` button to add the changes (_values.yaml_).

Type `Update customerDatabase` in the comment. Then, commit it (e.g. _Command + Enter on macOS_) and click on `Sync Changes`.

image::vscode-commit.png[vscode-commit]

Access link:https://gitea.apps.%SUBDOMAIN%/%USERID%/app-mod-projects[your Gitea admin console^] with the following credentials:
