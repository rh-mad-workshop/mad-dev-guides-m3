= 2. Build CI/CD Pipelines - 35 minutes
...
Access link:https://argocd-server-retail-%USERID%.apps.%SUBDOMAIN%[ArgoCD web console^]. Click on `LOG VIA OPENSHIFT` button.
...
Go back to the OpenShift web console, reveal the link:https://console-openshift-console.apps.%SUBDOMAIN%/k8s/ns/retail-%USERID%/secrets/customers-secret[customers-secret^]. You will see the updated the JDBC configurations.
...
First, open a new browser to access the link:https://console-openshift-console.apps.%SUBDOMAIN%/dev-pipelines/ns/cicd-%USERID%[OpenShift Pipleline^].

Use following credential if you haven't logged in to the OpenShift cluster before. 
...
Go back to the link:https://console-openshift-console.apps.%SUBDOMAIN%/topology/ns/retail-%USERID%?view=graph[Topology View^] in `retail-%USERID%` project at Developer perspective, the applications deployment should look like:
